...
/**
 * Season-by-season table. Numerals are condensed and right-aligned so the
 * columns line up on the decimal — the previous proportional figures made every
 * row look slightly misaligned.
 */
export function CareerStatsTable({ stats }: { stats: CareerStat[] }) {
  if (stats.length === 0) {
    return (
      <Text className="font-sans text-[13px] text-slate">No season stats recorded yet.</Text>
    );
  }
      ...
      {stats.map((stat, index) => (
        <View
          ...
          <View className="flex-[2.3] pr-2">
            <Text className="font-stat text-[16px] tracking-stat text-ink">{stat.season}</Text>
          ...
          {COLUMNS.map((column) => (
            <Text
              key={column.key}
              className="font-stat flex-1 text-right text-[19px] tracking-stat text-ink">
