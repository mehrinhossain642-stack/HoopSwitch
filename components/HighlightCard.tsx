...
export function HighlightCard({ highlight }: { highlight: Highlight }) {
  return (
    <Touchable
      ...
      <View
        className="w-full overflow-hidden rounded-md border border-border bg-chrome"
        ...
        {/* Scrim only along the bottom, so the play button stays on the image and
            the duration chip stays legible on a bright frame. */}
        <View
        ...
        <View
          className="absolute bottom-2 right-2 rounded px-1.5"
          style={{ backgroundColor: 'rgba(13,14,17,0.8)' }}>
          <Text className="font-stat text-[13px] tracking-stat text-white">
