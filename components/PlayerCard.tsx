...
/**
 * Candidate card. Production leads — a coach scans PPG/RPG/APG first and only
 * then checks whether the frame fits the slot — so the stat strip sits directly
 * under the name and the physicals follow as supporting spec.
 */
export function PlayerCard({
  ...
  const eligibility = `${player.eligibility_years} yr${player.eligibility_years === 1 ? '' : 's'} eligibility left`;
      ...
      <Card bare rail={match ? tierColor(match.tier) : undefined}>
        ...
        <View className="flex-row items-center border-t border-border bg-bg px-4 py-3">
          {match ? (
            ...
            <Text className="font-sans flex-1 text-[12px] text-slate">Not scored for this slot.</Text>
          )}
