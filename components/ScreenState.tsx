...
/**
 * First-load state. Renders the feed's own shape as skeletons instead of a
 * centred spinner, so arriving data doesn't reflow the screen.
 */
export function ScreenLoading({ label = 'Loading' }: { label?: string }) {
  return (
    <Screen>
      <Content className="pt-4" accessibilityLabel={`${label}…`}>
        <View className="mb-4">
          <View className="h-4" />
        </View>
        <FeedSkeleton />
    ...
    <Screen>
      <View className="flex-1 items-center justify-center">
        <Content className="items-center">
          ...
          <Text
            className="font-sans mt-2 max-w-[380px] text-center text-[13px] leading-[19px] text-slate">
            {message}
          </Text>
 ...
 * forward is the point where a user decides the app is broken.
 */
