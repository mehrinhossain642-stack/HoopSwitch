 ...
 * stat line — so the top of the screen reads as one surface and the stats get the
 * contrast they deserve.
 */
export function ProfileHero({
  ...
  meta: string;
  /** Optional second line, e.g. "Head coach · A. Miller". */
  submeta?: string;
  ...
  return (
    <View className="bg-chrome" style={{ paddingTop: isDesktop ? 20 : insets.top + 6 }}>
      <View
        className="w-full self-center pb-4"
        style={{ maxWidth: CONTENT_MAX_WIDTH, paddingHorizontal: gutter }}>
        <View className="mb-3.5 flex-row items-center justify-between">
          <Text className="font-stat text-[14px] tracking-eyebrow text-chrome-text-muted">
            {eyebrow.toUpperCase()}
          </Text>
          {action}
        </View>
        ...
        <View className="flex-row items-center">
          ...
          <View className="ml-4 flex-1">
            <View className="flex-row items-center">
              <Text
                className="font-display flex-shrink text-[22px] leading-[27px] text-chrome-text"
                numberOfLines={2}
            ...
            <Text className="font-sans mt-1 text-[12px] leading-[17px] text-chrome-text-muted">
              {meta}
            </Text>
            {submeta ? (
              <Text className="font-sans-medium mt-0.5 text-[12px] text-chrome-text">
                {submeta}
              </Text>
            ) : null}
          </View>
