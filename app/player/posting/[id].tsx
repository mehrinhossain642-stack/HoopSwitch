...
export default function PostingDetail() {
  ...
  const contentStyle = useContentContainerStyle({ paddingBottom: STICKY_BAR_CLEARANCE });
  ...
  return (
    <Screen edges={[]}>
      ...
      <ScrollView contentContainerStyle={{ ...contentStyle, paddingTop: 16 }}>
        ...
        {/* Fit is the reason this screen exists, so it gets its own panel above
            the requirements rather than a chip somewhere in the middle. */}
        {match ? (
          <>
      ...
      <StickyActionBar>
        {match ? (
          <View className="mr-3 flex-1">
            <FitScore score={match.score} tier={match.tier} />
          </View>
        ) : (
          <View className="mr-3 flex-1" />
        )}
...
function ScoreRow({ label, value, last }: { label: string; value: number; last: boolean }) {
  ...
  // Thresholds mirror the server's own tiering so a row's colour agrees with
  // the headline score.
  const tone = value >= 0.78 ? tierColor('good') : value >= 0.4 ? tierColor('partial') : COLORS.danger;

  return (
    <View className={last ? '' : 'mb-3.5'}>
      <View className="mb-1.5 flex-row items-center justify-between">
        <Text className="font-sans-medium text-[13px] text-ink">{label}</Text>
        <Text className="font-stat text-[17px] tracking-stat" style={{ color: tone }}>
          {pct}%
        </Text>
      </View>
